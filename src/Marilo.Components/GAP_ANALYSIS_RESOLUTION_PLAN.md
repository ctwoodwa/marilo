...
| MariloIcon                | 3         | single-pass | No                                             | **T1 - Critical** | ✅ CLOSED 2026-04-02 — doc-only; icon system upgrade complete — Tabler Icons default, multi-provider architecture. Stage 06 report: `gap-icon-closure-report.md`. Docs updated in Phase 4 Enforcement (E-01).        |
...
| MariloEditor              | 54        | multi-pass  | Yes (ProseMirror / rich-text engine)           | **T2 - High**     | ✅ CLOSED 2026-04-10 — all gaps resolved. Batch 1: validation/tools/14 tests; Batch 2a: Markdig import/export; Adaptive toolbar; JS Interop Batch 3: table/image resize. Stage 06 reports: `gap-editor-batch1-closure-report.md`, `gap-editor-batch2a-closure-report.md`, `gap-editor-table-image-resize-closure-report.md`. Security model docs added Phase 4 Enforcement (E-02). |
...
| MariloTreeView            | 6+16      | multi-pass  | Yes (expanded/select`e`d binding, drag-drop) | **T2 - High**     | ✅ CLOSED 2026-04-02 — 21/22 gaps resolved; Stage 06 report: `gap-treeview-closure-report.md`. **DEFERRED: Gap 18 (Virtualization)** — requires flatten-and-virtualize architecture rebuild; deferred to future iteration. |
...
- GAP-treeview Gap 18 (Virtualization) → **DEFERRED** — Future iteration — requires flatten-and-virtualize architecture change (closure report: `gap-treeview-closure-report.md`, 2026-04-02)
- GAP-MSEL-007 ScrollMode → **DEFERRED** — Blazor `<Virtualize>` lacks scroll-mode primitive; custom virtualization rebuild required; not implemented as a no-op (closure report: `gap-t4-picker-batch6-closure-report.md`, 2026-04-08)
- GAP-editor JS interop (table resize, image resize) → **CLOSED 2026-04-11** — implemented in JS Interop Batch 3 (closure report: `gap-editor-table-image-resize-closure-report.md`)
...
| GAP-MSEL-007         | MariloMultiSelect | ItemHeight + PageSize parameters wired through to `<Virtualize>` element. **DEFERRED: ScrollMode** — Blazor `<Virtualize>` lacks the primitive; custom virtualization rebuild required. | 03→05→06 | **Resolved** ✅ (4 bUnit tests; ScrollMode **DEFERRED** — requires custom virtualization rebuild; not a no-op, future iteration) |
