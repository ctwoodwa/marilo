# Parity Score Rubric -- MariloSplitter
...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (horizontal split default, resize handle hover, resize handle active) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (min/max constraint hit, three-pane layout) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** horizontal split default, resize handle hover, resize handle active, collapsed pane
- **Secondary:** vertical split default, collapse button styling, nested splitters
- **Edge:** min/max constraint hit, three-pane layout
...
| Missing-state coverage | The state is not implemented at all (e.g., no collapse button, no active handle treatment) | Gap-analysis intake for implementation |
...
- Handle states are high-visibility — even subtle color or sizing mismatches on hover/active should score 1 or 2, not 3.
