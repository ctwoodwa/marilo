# Parity Score Rubric -- MariloDockManager
...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (docked panel, panel header, tab strip) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (drag preview, drop indicator, empty dock zone) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** docked panel, panel header, tab strip
- **Secondary:** floating panel, split layout, close/minimize buttons
- **Edge:** drag preview, drop indicator, empty dock zone
...
| Missing-state coverage | The state is not implemented at all (e.g., no drop indicator style) | Gap-analysis intake for implementation |

## Scoring Guidelines

- Score the Marilo rendering against the internal Marilo delivery-quality baseline. There is no Telerik Blazor equivalent for DockManager.
