...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (toolbar default, content area idle, toolbar button hover) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (source view, readonly mode) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** toolbar default, toolbar button hover, toolbar button active, content area idle
- **Secondary:** toolbar button focus, toolbar separator, formatting active, placeholder, link dialog, image dialog
- **Edge:** source view, readonly mode
...
| Missing-state coverage | The state is not implemented at all (e.g., no toolbar button active style) | Gap-analysis intake for implementation |
...
- Toolbar button sizing and focus ring visibility are precision-sensitive — score strictly for accessibility-relevant gaps.
