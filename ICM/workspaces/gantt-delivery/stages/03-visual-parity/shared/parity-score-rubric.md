...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (default task bar, task hover, task selected) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (editing row, dependency lines at edge viewports) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** default task bar, task hover, task selected, tree column idle
- **Secondary:** summary bar, milestone diamond, progress indicator, expanded row, collapsed row, timeline header, current date line, dependency lines
- **Edge:** editing row
...
| Missing-state coverage | The state is not implemented at all (e.g., no dependency line style) | Gap-analysis intake for implementation |
...
- Milestone diamond size and dependency line weight are precision-sensitive — score strictly.
