# Parity Score Rubric -- MariloDataSheet
...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (cell grid default, selected cell, cell editing) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (formula bar, sheet tabs, empty sheet) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** cell grid default, selected cell, cell editing
- **Secondary:** column/row headers, frozen rows/columns, cell range selection
- **Edge:** formula bar, sheet tabs, empty sheet
...
| Missing-state coverage | The state is not implemented at all (e.g., no frozen separator style) | Gap-analysis intake for implementation |

## Scoring Guidelines

- Score the Marilo rendering against the internal Marilo delivery-quality baseline. There is no Telerik equivalent for DataSheet.
