...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (default view, file grid pane, item hover, selected item) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (empty folder, upload area) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** default view, file grid pane, item hover, selected item
- **Secondary:** tree navigation pane, pane splitter, breadcrumb, toolbar idle, search input, file/folder icons, context menu, tree expanded/collapsed
- **Edge:** upload area, empty folder
...
| Missing-state coverage | The state is not implemented at all (e.g., no context menu hover style) | Gap-analysis intake for implementation |
...
- Context menu chrome and pane splitter are high-visibility structural elements — score strictly.
