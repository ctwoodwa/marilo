...
| Polish | Score 2 on edge states (empty, loading, templated) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** default hierarchy, hovered node, focused node, selected node
- **Secondary:** disabled node, expanded/collapsed, checkbox checked/unchecked
- **Edge:** load-on-demand, empty tree, templated node, checkbox indeterminate
...
| Missing-state coverage | The state is not implemented at all (e.g., no indeterminate checkbox visual) | Gap-analysis intake for implementation |
