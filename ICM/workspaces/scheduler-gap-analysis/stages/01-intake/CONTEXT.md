...
| Target project | src/Marilo.Components/ | File listing (2 levels deep) | Understand project structure |
...
3. Normalize each gap into the standard record format from `shared/gap-record-format.md`. Assign a unique ID: `GAP-SCHEDULER-[NNN]` (e.g., `GAP-SCHEDULER-001`).
...
5. Identify cross-cutting themes (gaps that repeat across multiple areas). Tag each gap with its theme(s).
...
|-------|---------------|
| Every gap has a unique ID | No duplicate GAP-SCHEDULER-* IDs in the inventory |
...
|----------|----------|--------|
| Gap inventory | output/gap-scheduler-inventory.md | Normalized gap records grouped by area/theme |
