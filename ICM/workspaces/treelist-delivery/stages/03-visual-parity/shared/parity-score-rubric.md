...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (default view, row hover, selected row, hierarchical rows) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (empty, loading) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** default view, hierarchical rows, row hover, selected row
- **Secondary:** header row, expanded row, collapsed row, checkbox selection, sorted column, filter row, inline edit row, pager idle, pager hover/active
- **Edge:** empty state, loading state
...
| Missing-state coverage | The state is not implemented at all (e.g., no filter row style) | Gap-analysis intake for implementation |
...
- Tree indentation and expand icon alignment are precision-sensitive — score strictly for hierarchical rows.
