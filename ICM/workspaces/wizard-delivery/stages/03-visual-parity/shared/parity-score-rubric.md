# Parity Score Rubric -- MariloWizard
...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (step indicator active, step content, navigation buttons) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (validation error, non-linear mode) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** step indicator active, step content area, navigation buttons (prev/next/finish), button alignment
- **Secondary:** step indicator completed, step indicator pending, connector line, disabled step
- **Edge:** validation error, non-linear mode
...
| Missing-state coverage | The state is not implemented at all (e.g., no validation error indicator on step) | Gap-analysis intake for implementation |
...
- Step indicator active state is high-visibility — even subtle sizing or color mismatches should score 1 or 2, not 3.
