...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (default view, row headers, column headers, data cells) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (empty, loading, field chooser drag) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** default view, row headers, column headers, data cells
- **Secondary:** aggregation typography, expanded group, collapsed group, group expand/collapse icon, field chooser idle
- **Edge:** empty state, loading state, field chooser drag
...
| Missing-state coverage | The state is not implemented at all (e.g., no group expand icon style) | Gap-analysis intake for implementation |
...
- Header nesting indentation and aggregation typography distinction are precision-sensitive — score strictly.
