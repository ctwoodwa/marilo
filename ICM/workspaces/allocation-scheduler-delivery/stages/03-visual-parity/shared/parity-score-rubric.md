...
|----------|------------|--------|
| Critical | Score 0-1 on a primary state (default view, cell hover, selected allocation) in any theme | Must fix before delivery gate |
...
| Polish | Score 2 on edge states (empty resource, loading) | Backlog for next phase |

## Primary vs. Secondary vs. Edge States

- **Primary:** default view, occupied cell, cell hover, selected allocation
- **Secondary:** disabled cell, cell editing, drag-fill in progress, conflict indicator, current period highlight, splitter at rest, timeline header
- **Edge:** empty resource, loading state
...
| Missing-state coverage | The state is not implemented at all (e.g., no conflict indicator style) | Gap-analysis intake for implementation |

## Scoring Guidelines

- Score the Marilo rendering against the reference at the same theme and mode. Do not cross-compare themes.
- If no Telerik reference exists for a state (e.g., drag-fill preview), score against internal Marilo delivery-quality expectations.
...
- Dark mode tint issues caused by color-mix base color are a known systemic issue — score them honestly but note the known cause.
