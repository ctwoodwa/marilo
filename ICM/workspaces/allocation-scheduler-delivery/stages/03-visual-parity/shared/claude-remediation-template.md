...
**Gap ID:** VP-allocation-scheduler-[sequence]
...
| [SCSS component file — allocation-scheduler] | [specific selector or rule, e.g., .marilo-alloc-cell, .marilo-resource-panel] |
...
- [ ] Other states of this component (e.g., fixing cell hover should not break selected allocation)
- [ ] Other components (if changing a foundation token, scan for consumers)
- [ ] Splitter layout — splitter alignment is sensitive to flex/grid container changes
...
- Dark mode color-mix fixes must use var(--marilo-color-surface) as the base color, not hardcoded #ffffff
