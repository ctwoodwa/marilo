# Visual Parity -- MariloAllocationScheduler

Compare MariloAllocationScheduler rendering against Telerik Scheduler (resource scheduling variant) visual baseline across Bootstrap, Fluent, and Material themes in light and dark modes. Produce structured gap records and remediation-ready outputs.

## When to Enter

- After Stage 02 (Example UX) has produced demo scenarios
- When the `parity` trigger is invoked
- When visual quality of the AllocationScheduler needs evaluation against reference
...
| Capture matrix | shared/capture-matrix.md | Full file | AllocationScheduler-specific theme/mode/state combinations |
...
**Telerik Scheduler (resource scheduling variant) parity.** The Telerik Blazor Scheduler configured for resource allocation is the visual reference baseline for AllocationScheduler scenarios including resource panels, timeline headers, bucket cells, drag-fill, and grouped timelines. Marilo does not need to clone every Telerik behavior — the comparison targets visual quality, density, and state treatment.

## Process

1. Read the capture matrix — AllocationScheduler has ~14 primary state/scenario combinations.
...
7. Write output/allocation-scheduler-visual-parity-gaps.md with all gap records.
8. Write output/allocation-scheduler-parity-summary.md with overall scores and coverage.
...
|----------|----------|--------|
| Visual parity gaps | output/allocation-scheduler-visual-parity-gaps.md | visual-parity-gap-format.md |
| Parity summary | output/allocation-scheduler-parity-summary.md | Summary with scores per theme/mode |
| Visual parity plan | output/allocation-scheduler-visual-parity-plan.md | Starter plan (seeded at stage creation) |
