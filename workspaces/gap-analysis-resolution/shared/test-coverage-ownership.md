...
This document makes the ownership model explicit for test evidence across the gap-analysis-resolution pipeline.
...
- Format: `gap-[slug] → tests/path/to/File.test.tsx :: "test description"`.
- If tests are deferred, the log MUST state the reason and create a follow-up entry (e.g., "manual-only: no bUnit harness available; tracked in gap-[slug]-followup").
...
Each arrow is a contract: the downstream stage reads the upstream stage's output and holds it to account.
