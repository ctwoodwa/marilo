...
* The [`Marilo.AI.SmartComponents.Extensions` NuGet package](https://www.nuget.org/packages/Marilo.AI.SmartComponents.Extensions) installed (if using the Marilo AI service integration)
* An `HttpClient` instance configured for making requests to your AI service

For detailed guidance on implementing a compatible backend service, see [Integration with Marilo.AI.SmartComponents.Extensions](slug:grid-ai-service-setup).

## Basic Setup

### Add Required NuGet Package

Install the `Marilo.AI.SmartComponents.Extensions` package to your project:
...
The following example demonstrates the recommended pattern for AI service communication used in the Marilo demos:
    ...
    private async Task OnAIPromptRequest(GridSmartBoxAIAssistantPromptRequestEventArgs args)
    {
        try
        {
            var requestResult = await HttpClientInstance.PostAsJsonAsync(
                "https://demos.marilo.com/service/v2/ai/grid/smart-state", 
                args.Request);
...
If you have implemented your own backend service using the `Marilo.AI.SmartComponents.Extensions` library, point to your own endpoint:
