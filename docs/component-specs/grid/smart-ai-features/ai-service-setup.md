...
previous_url: /common-features/integration-with-marilo-ai-smartcomponents-extensions
tags: marilo,blazor,grid,ai,service,setup,smart,extensions
published: True
position: 10
components: ["grid"]
---
...
To enable AI-powered interaction in the [Smart Grid AI Assistant tools](slug:grid-ai-assistant-tools-setup), you need a backend service that processes natural language prompts and returns executable Grid commands. The `Marilo.AI.SmartComponents.Extensions` library for .NET simplifies this by automatically handling the request/response format and command generation.

This article shows you how to build your own .NET backend service. You will learn how to:

- Install the [`Marilo.AI.SmartComponents.Extensions`](https://www.nuget.org/packages/Marilo.AI.SmartComponents.Extensions) package
...
The AI Assistant tools send user prompts to your backend service, which must return the response in a specific format that the Grid understands. The `Marilo.AI.SmartComponents.Extensions` package for .NET simplifies this process by handling the request/response formatting automatically.
...
Add the [`Marilo.AI.SmartComponents.Extensions` NuGet package](https://www.nuget.org/packages/Marilo.AI.SmartComponents.Extensions) to your project. It adds the following dependencies:

```bash.skip-repl
dotnet add package Marilo.AI.SmartComponents.Extensions
