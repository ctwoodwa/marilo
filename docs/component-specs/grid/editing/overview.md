...
tags: marilo, blazor, grid, editing
published: True
...
The Marilo Grid for Blazor supports create, update, and delete operations (CRUD) with different modes and user experience. The component also supports built-in `DataAnnotations` or custom validation. This page describes:
...
1. Set the `EditMode` parameter to a [member of the `GridEditMode` enum](slug:marilo.blazor.grideditmode). The default `EditMode` parameter value is `None` and the built-in [`Add` and `Edit` commands](slug://components/grid/columns/command#built-in-commands) don't work.
...
* By using the [Grid keyboard navigation](https://demos.marilo.com/blazor-ui/grid/keyboard-navigation).
...
You can control whether a newly added item appears at the top or bottom of the Grid. Use the [`NewRowPosition`](https://www.marilo.com/blazor-ui/documentation/api/marilo.blazor.gridnewrowposition) parameter to specify the position. This parameter does not affect Popup edit mode, which always displays a dialog for new items.
...
* [Online Grid Demos](https://demos.marilo.com/blazor-ui/grid/editing-inline)
