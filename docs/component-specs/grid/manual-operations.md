...
tags: marilo,blazor,grid,read,filter,sort,page,manual,data,data source
published: True
position: 55
components: ["grid"]
---
...
* [Use OData Service](https://github.com/marilo/blazor-ui/tree/master/grid/odata)
* [Serialize the DataSoureRequest to the server](https://github.com/marilo/blazor-ui/tree/master/grid/datasourcerequest-on-server)
* [Debounce Data Source Operations and Requests](slug:grid-kb-debounce-operations)

## Custom paging with a remote service

>note The example below demonstrates how to use just Paging with a remote service. For a more complex setup including other Grid functionalities (such as sorting, filtering etc.) you can check [this project for using Marilo DataSourceRequest and DataSourceResult on the server](https://github.com/marilo/blazor-ui/tree/master/grid/datasourcerequest-on-server) in our public repository.
...
@code {
	...
	//https://docs.marilo.com/blazor-ui/components/grid/editing/overview
...
## Marilo .ToDataSourceResult(request)

If you have all the data at once, the Marilo .ToDataSourceResult(request) extension method can manage the operations for you.

>tip You can find examples of how to use this object to easily retrieve data on the server in a performant manner in the following repo: [https://github.com/marilo/blazor-ui/tree/master/grid/datasourcerequest-on-server](https://github.com/marilo/blazor-ui/tree/master/grid/datasourcerequest-on-server).
>
> We support the `System.Text.Json` serialization that is built-in in Blazor.

>caption Use Marilo .ToDataSourceResult() extension method to filter, sort and page data.

````RAZOR
Using Marilo DataSource extension methods to manipulate all the data into paged chunks and also perform other operations like filtering, sorting, etc. There is a deliberate delay in the data source operations in this example to mimic real life delays and to showcase the async nature of the calls.
...
When the Grid needs to be grouped, the shape of the data changes and it is no longer a flat list of models. Instead, the data is a nested list of [`AggregateFunctionsGroup` objects](slug:marilo.datasource.aggregatefunctionsgroup) that describe each group and include its data items.
...
````RAZOR
This sample shows how to set up the grid to use grouping with manual data source operations, and how to use the Marilo DataSource extensions to prepare grouped data.
...
@code {
    ...
    protected async Task ReadItems(GridReadEventArgs args)
    {
        // in this example, we use the Marilo extension methods to shape the data
        // you can, instead, call a service, read more in the following example projects
        // https://github.com/marilo/blazor-ui/tree/master/grid/datasourcerequest-on-server
        var datasourceResult = SourceData.ToDataSourceResult(args.Request);
...
* [Live Demo: Manual Data Source Operations](https://demos.marilo.com/blazor-ui/grid/manual-operations)
* [Use OData Service](https://github.com/marilo/blazor-ui/tree/master/grid/odata)
* [Custom Server Operations](https://github.com/marilo/blazor-ui/tree/master/grid/datasourcerequest-on-server)
* [DataSourceRequest Object API](slug:Marilo.DataSource.DataSourceRequest)
* [DataSourceResult Object API](slug:Marilo.DataSource.DataSourceResult)
