...
The Marilo Blazor Spreadsheet fires events that are related to different user actions. This article describes all events and event arguments.
...
````RAZOR
<p><label><MariloCheckBox @bind-Value="@ShouldCancelEvents" /> Cancel Spreadsheet Events</label></p>

<p>Spreadsheet Event Log: @( new MarkupString(SpreadSheetEventLog) )</p>

<MariloSpreadsheet OnDownload="@OnSpreadsheetDownload"
...
@code {
    ...
    private void OnSpreadsheetDownload(SpreadsheetDownloadEventArgs args)
    {
        ...
        args.FileName = $"marilo-spreadsheet-{now.ToString("HH-mm-ss")}";
...
* [Live Demo: Spreadsheet Events](https://demos.marilo.com/blazor-ui/spreadsheet/events)
