...
The Marilo Blazor Spreadsheet organizes all its tools in a tool set with one or more tool set items. This article describes the built-in Spreadsheet tools and how to define custom tools.
...
The Spreadsheet uses Marilo Blazor components to create its built-in tool sets and tools. You can add, remove, and reorder tools in the built-in tool sets. You can also customize the exposed tool properties.

The example below shows how to:

* Get the **File** and **Home** tool set items. The built-in tool set items are distinguishable by their index or `Title`. The title can change in localized applications, so you may need to use the [`IMariloStringLocalizer` service](slug:globalization-localization) to search for the localized `Title` string.
...
````RAZOR
@using Marilo.Blazor.Components.Spreadsheet
@using Marilo.Blazor.Components.Spreadsheet.ToolBar.ToolTypes
@using Marilo.Blazor.Resources
@using Marilo.Blazor.Services

@* Needed to find the built-in Home tool by its localized title if the application is using more than one language *@
@* @inject IMariloStringLocalizer Localizer *@

<MariloSpreadsheet Tools="@DefaultToolsWithCustomizations">
</MariloSpreadsheet>
...
<MariloSpreadsheet Tools="@SpreadsheetTools">
</MariloSpreadsheet>
...
@code {
    ...
    private RenderFragment CustomToolFragment =>
        @<MariloButton Icon="@SvgIcon.FileExcel"
...
* [Live Demo: Spreadsheet Tools](https://demos.marilo.com/blazor-ui/spreadsheet/tools)
