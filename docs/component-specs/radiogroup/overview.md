...
tags: marilo,blazor,radiobuttongroup,radio,list,overview
published: True
position: 0
...
The <a href="https://www.marilo.com/blazor-ui/radiogroup" target="_blank">Blazor RadioGroup component</a> allows the user to select an option from a predefined set of choices in a list of radio buttons. The radio group is styled according to the Marilo [Theme](slug:themes-overview). You can also choose the [layout order](slug:radiogroup-layout) and [label position](slug:radiogroup-label-position).

## Creating Blazor RadioGroup

1. Add the `<MariloRadioGroup>` tag to a Razor file.
...
<MariloRadioGroup Data="@GenderOptions"
...
@code{
    MariloRadioGroup<GenderModel, int?> RadioGroupRef { get; set; }
...
````RAZOR
<MariloButton OnClick="@FocusLastRadioItem">Focus Last Radio Item</MariloButton>
...
@code{
    private MariloRadioGroup<ListItem, int?>? RadioGroupRef { get; set; }
...
* [Live RadioGroup Demos](https://demos.marilo.com/blazor-ui/radiogroup/overview)
* [RadioGroup API Reference](slug:Marilo.Blazor.Components.MariloRadioGroup-2)
