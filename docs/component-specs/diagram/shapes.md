...
The Shape is the main building block of the Marilo Diagram for Blazor. It represents a single node in the graph. This article describes all Diagram Shape types and customization options.

## Basics

The fundamental settings of the Marilo Diagram Shapes include:
...
The available Diagram Shape types are the members of the [`DiagramShapeType` enum](slug:Marilo.Blazor.DiagramShapeType). The default Shape type is `Rectangle`.
...
````RAZOR
<MariloDiagram Height="440px" Zoom="0.5">
...
1. Get familiar with the [related JavaScript API and available visual primitives](https://www.marilo.com/kendo-jquery-ui/documentation/api/javascript/dataviz/ui/diagram/configuration/shapedefaults.visual).
1. Implement a JavaScript function that returns a [`MariloBlazor.DiagramCommon.Group` JavaScript object](https://www.marilo.com/kendo-jquery-ui/documentation/api/javascript/dataviz/diagram/group). The `Group` can contain any number of other primitives like `Circle`, `Image`, `Line`, `Rectangle`, `TextBlock`, and others.
...
1. To align or center primitives automatically, use a [`Layout` primitive](https://www.marilo.com/kendo-jquery-ui/documentation/api/javascript/dataviz/diagram/layout) as a parent container. Make sure to `reflow()` the `Layout` object after adding children.
...
> This section links to the documentation of Kendo UI for jQuery. The Marilo Diagram for Blazor is not a wrapper of the Kendo UI Diagram. However, both components use the same client-side rendering engine. When the Kendo UI documentation mentions the `kendo.dataviz.diagram` JavaScript namespace, you must use `MariloBlazor.DiagramCommon` instead.

In addition to the following example, also check the [Blazor Diagram Overview demo](https://demos.marilo.com/blazor-ui/diagram/overview) and the [Show Diagram Shape Text Below Image](slug:diagram-kb-show-shape-text-below-image) knowledge base article. Both use a visual function.
...
<script suppress-error="BL9992">
    function shapeVisualFunction(context) {
        let diagramNS = MariloBlazor.DiagramCommon;
...
* [Live Demo: Diagram Layout and Shape Types](https://demos.marilo.com/blazor-ui/diagram/configuration)
* [Live Demo: Using Diagram Visuals](https://demos.marilo.com/blazor-ui/diagram/overview)
