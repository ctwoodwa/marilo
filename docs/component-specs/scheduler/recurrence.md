...
description: Learn how to set up the Marilo Scheduler for Blazor to display and edit recurring appointments.
slug: scheduler-recurrence
tags: marilo, blazor, scheduler, recurrence
published: True
position: 7
...
The Marilo Scheduler for Blazor supports displaying and editing of recurring appointments and exceptions. This article describes how to:
...
````RAZOR
<MariloScheduler Data="@SchedulerData"
...
Marilo UI for Blazor provides standalone components that you can use to edit recurring appointments outside the Scheduler or in a [custom Scheduler popup edit form](slug:scheduler-kb-custom-edit-form).
...
* A `Rule` parameter of type `Marilo.Recurrence.RecurrenceRule` that supports two-way binding.
...
### Marilo Form Integration

There are two recommended ways to use the Marilo recurrence editors in a Marilo Form:
...
The following examples can serve as a reference for creating [custom Marilo Scheduler edit forms](slug:scheduler-kb-custom-edit-form) with recurrence editing. Using a markup structure that differs from the ones below may produce unexpected layouts.

>caption Using Marilo recurrence editors in separate Form item templates

````RAZOR
@using Marilo.Recurrence

<MariloForm Model="@RecurringAppointment"
    ...
    <FormItems>
        ...
        <FormItem>
            <Template>
                <MariloRecurrenceFrequencyEditor Rule="@Rule"
        ...
        <FormItem>
            <Template>
                @if (Rule != null)
                {
                    <MariloRecurrenceIntervalEditor Rule="@Rule" />
                }
            </Template>
        </FormItem>
        <FormItem>
            <Template>
                <MariloRecurrenceEditor Rule="@Rule"
                                         RuleChanged="@OnRuleChanged" />
            </Template>
        </FormItem>
        <FormItem>
            <Template>
                @if (Rule != null)
                {
                    <MariloRecurrenceEndEditor Rule="@Rule"
...
To add the recurrence editors to a `FormItemsTemplate`, follow the same approach as in the example above, but add the following `<FormItemsTemplate>` tag as a child of `<MariloForm>`.

>caption Using Marilo recurrence editors in a FormItemsTemplate
...
<FormItemsTemplate Context="formContext">
    ...
    <MariloFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.Title)) )" />
    <MariloFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.Start)) )" />
    <MariloFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.End)) )" />
    <MariloFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Appointment.RecurrenceRule)) )" />
...
* [Live Demo: Scheduler Recurring Appointments](https://demos.marilo.com/blazor-ui/scheduler/recurring-appointments)
