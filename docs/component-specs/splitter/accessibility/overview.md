...
page_title: Marilo UI for Blazor Splitter Documentation | Splitter Accessibility Overview
description: "Get started with the Marilo UI for Blazor Splitter and learn about its accessibility support for WAI-ARIA, Section 508, and WCAG 2.2."
tags: marilo,blazor,accessibility,wai-aria,wcag,splitter
...
The illustrative example below shows the tile resize action, achievable through the [Context Menu](slug:contextmenu-overview). Marilo UI for Blazor to offer a versatile API that allows users to trigger all functions programmatically or externally, meeting diverse accessibility requirements for any applications.
...
@using Marilo.SvgIcons

<MariloContextMenu @ref="@ContextMenuRef"
                    Data="@MenuItems"
                    OnClick="@((MenuItem item) => ContextMenuClickHandler(item))">
    <ItemTemplate Context="item">
        @{
            <MariloSvgIcon Icon="@item.Icon" />
            <div>@item.Text</div>

            @if (item.CommandName == "ResizePane")
            {
                <div style="margin-left: 5px;">
                    <MariloSvgIcon Icon="@SvgIcon.WindowRestore" />
                </div>
            }
        }
    </ItemTemplate>
...
<MariloSplitter AriaLabel="My Splitter" Width="500px" Height="300px" Orientation="@SplitterOrientation.Horizontal">
    <SplitterPanes>
...
<MariloDialog @bind-Visible="@Visible"
               Class="dialog-btn-formatting"
               Width="300px"
               Title="Resize">
    <DialogContent>
        <label id="newWidth" for="newWidth" style="display: block;">New width:</label>
        <MariloNumericTextBox AriaLabelledBy="newWidth" ShowClearButton="true" Width="100px" @bind-Value="NewWidth" />
        <MariloComboBox AriaLabel="Set unit"
                         Data="@ComboBoxData"
                         Width="100px"
    ...
    <DialogButtons>
        <MariloButton AriaLabel="Confirm Resize" OnClick="@OnApplyClick" ThemeColor="@ThemeConstants.Button.ThemeColor.Primary"><MariloSvgIcon Icon="@SvgIcon.Check" /><span style="margin-left: 5px;">Resize</span></MariloButton>
        <MariloButton AriaLabel="Cancel Resize" OnClick="@(() => { Visible = false; })"><MariloSvgIcon Icon="@SvgIcon.CancelOutline" /><span style="margin-left: 5px;">Cancel</span></MariloButton>
    </DialogButtons>
</MariloDialog>
...
## See also
 * [Live demo: Splitter Accessibility](https://demos.marilo.com/blazor-ui/splitter/keyboard-navigation)