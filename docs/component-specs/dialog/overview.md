...
tags: marilo,blazor,dialog,overview
published: True
position: 0
components: ["dialog"]
---
...
1. Add the `MariloDialog` tag to a Razor file.
...
5. (optional) Configure the [`DialogButtons`](slug:dialog-action-buttons) inside the `MariloDialog` tag.

>caption A basic configuration of the Marilo Dialog.
...
| `ThemeColor` | `string` | A predefined color scheme for the Dialog, especially the titlebar. Use the available members of the static class [`ThemeConstants.Dialog.ThemeColor`](slug:Marilo.Blazor.ThemeConstants.Dialog.ThemeColor). |
...
<MariloButton  OnClick="OpenDialog">Open Dialog</MariloButton>

<MariloDialog @ref="DialogRef" @bind-Visible="_dialogVisible">
    <DialogContent>
        <p role="status">Current count: @_currentCount</p>
    </DialogContent>
    <DialogButtons>
        <MariloButton OnClick="IncrementCount">Increment Count</MariloButton>
        <MariloButton OnClick="@(() => { _dialogVisible = false; })">Close</MariloButton>
    </DialogButtons>
</MariloDialog>

@code {
    MariloDialog DialogRef;
...
* [Live Dialog Demos](https://demos.marilo.com/blazor-ui/dialog/overview)
* [Dialog API Reference](slug:Marilo.Blazor.Components.MariloDialog)
