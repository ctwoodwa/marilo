...
tags: marilo,blazor,dialog,action,buttons
published: True
position: 7
components: ["dialog"]
---
...
````RAZOR
<MariloDialog @ref="@DialogRef"
               @bind-Visible="@DialogVisible"
               Title="Select Buttons Layout"
               ButtonsLayout="@SelectedButtonLayout">
    <DialogContent>
        <MariloRadioGroup Data="@ButtonLayouts"
                           @bind-Value="@SelectedButtonLayout"
                           OnChange="@OnRadioChange">
        </MariloRadioGroup>
    </DialogContent>
    <DialogButtons>
        <MariloButton>Cancel</MariloButton>
        <MariloButton ThemeColor="primary">OK</MariloButton>
    </DialogButtons>
</MariloDialog>
