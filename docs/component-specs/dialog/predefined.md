...
description: Predefined dialogs for Blazor - popup message, alert, confirm, and prompt. Explore the Marilo UI documentation.
slug: dialog-predefined
tags: marilo,blazor,dialog,predefined,alert,confirm,prompt
published: true
position: 2
components: ["dialog"]
---
...
Marilo UI for Blazor provides styled substitutes to the standard alert, confirm, and prompt dialogs of the browser. These Blazor popup messages match the theme of the components to make it obvious to the user that the modal dialog is coming from your application.


## Using Predefined Dialogs

To use the alert, confirm, and prompt dialogs (popup messages), receive a cascading parameter of type `Marilo.Blazor.DialogFactory`. This object exposes the methods you can use to display the predefined dialogs.
...
### MariloRootComponent Dependency

The `DialogFactory` cascading parameter can only be consumed inside child Razor components of the [`MariloRootComponent`](slug:rootcomponent-overview#purpose). This is easily achieved in standard Blazor apps where the `MariloRootComponent` resides inside the layout file.

* To show predefined dialogs in `MainLayout.razor`, [add the `MariloRootComponent` in a parent layout](slug:rootcomponent-overview#using-marilorootcomponent).
* When [using UI for Blazor in ASP.NET apps](slug:marilo-blazor-in-asp-net-app), an additional child component is needed.

If the `MariloRootComponent` is not available, or if it's at the wrong place in the app, [the `DialogFactory` will trigger a `NullReferenceException`](slug:dialog-kb-dialogfactory-null).

### Using await

The `DialogFactory` methods must be awaited. Do not use them with [discard variables (`_`)](https://learn.microsoft.com/en-us/dotnet/csharp/fundamentals/functional/discards). If you don't need to await the user response, then [use the `<MariloDialog>` component](slug:dialog-overview) declaratively. 
...
<MariloButton OnClick="@ShowAlert">Show Alert</MariloButton>
<MariloButton OnClick="@ShowAlertWithTitle">Show Alert with Custom Title</MariloButton>
<MariloButton OnClick="@ShowAlertWithTitleAndButton">Show Alert with Custom Title and Custom Button</MariloButton>
...
<MariloButton OnClick="@ShowConfirm">Show Confirm</MariloButton>
<MariloButton OnClick="@ShowConfirmWithTitle">Show Confirm with Custom Title</MariloButton>
<MariloButton OnClick="@ShowConfirmWithTitleAndButtons">Show Confirm with Custom Title and Custom Buttons</MariloButton>
...
<MariloButton OnClick="@ShowPrompt">Show Prompt</MariloButton>
<MariloButton OnClick="@ShowPromptWithTitle">Show Prompt with Custom Title</MariloButton>
<MariloButton OnClick="@ShowPromptWithTitleAndDefaultText">Show Prompt with Title and Default Input Text</MariloButton>
<MariloButton OnClick="@ShowPromptWithTitleDefaultTextAndButtons">Show Prompt with Title, Default Input Text and Custom Buttons</MariloButton>
...
* [Live Demo: Predefined Dialogs](https://demos.marilo.com/blazor-ui/dialog/predefined-dialogs)
* [Using the `<MariloDialog>` Component Declaratively](slug:dialog-overview)
