...
slug: dialog-header
tags: marilo,blazor,dialog,header
published: True
position: 5
components: ["dialog"]
---
...
>caption Title template and no close button in the Marilo Dialog.

````RAZOR
@* An example of a title template and hidden button for closing. *@

<MariloDialog @bind-Visible="@Visible" ShowCloseButton="false">
    <DialogTitle>
        <MariloSvgIcon Icon="@SvgIcon.CaretDoubleAltUp"></MariloSvgIcon>
        <strong>@Title</strong>
        <MariloSvgIcon Icon="@SvgIcon.CaretDoubleAltUp"></MariloSvgIcon>
    </DialogTitle>
    <DialogContent>
    ...
    <DialogButtons>
        <MariloButton OnClick="@(() => { Visible = false; })">Skip this version</MariloButton>
        <MariloButton OnClick="@(() => { Visible = false; })">Remind me later</MariloButton>
        <MariloButton OnClick="@(() => { Visible = false; })" ThemeColor="primary">Install update</MariloButton>
    </DialogButtons>
</MariloDialog>
