...
description: The Marilo DropDownTree component for Blazor enables you to select an item from a TreeView in a dropdown. Learn about the major DropDownTree features and functionality.
...
The [Blazor DropDownTree component](https://www.marilo.com/blazor-ui/dropdowntree) allows users to select a single value from a hierarchical list of dropdown items. The DropDownTree combines features of the [DropDownList](slug:components/dropdownlist/overview) and the [TreeView](slug:treeview-overview) components. Familiarity with the API of the two originating components is recommended.

## Creating Blazor DropDownTree

1. Add the `<MariloDropDownTree>` tag to your Razor page.
...
The Marilo DropDownTree fires events that enable the app to detect and react to user interactions with the component. Find out more about the [DropDownTree events and event arguments](slug:dropdowntree-events).

## DropDownTree API

Get familiar with all DropDownTree parameters, methods, events, and nested tags in the [DropDownTree API Reference](slug:Marilo.Blazor.Components.MariloDropDownTree-1).

## Popup Settings

The MariloDropDownTree exposes [settings for its dropdown (popup)](slug:Marilo.Blazor.Components.DropDownTreePopupSettings). To configure these options, declare a `<DropDownTreePopupSettings>` tag inside `<DropDownTreeSettings>`:
...
Add a reference to the component instance to use the [DropDownTree's methods](slug:Marilo.Blazor.Components.MariloDropDownTree-1). Note that the [DropDownTree is a generic component](slug:common-features-data-binding-overview#component-type). Its model type and value type must be part of the component reference definition.

````RAZOR.skip-repl
<MariloDropDownTree @ref="@DropDownTreeRef" />

@code {
    private MariloDropDownTree<int>? DropDownTreeRef;
...
* [DropDownTree API Reference](slug:marilo.blazor.components.marilodropdowntree-1)
* [Live Demo: DropDownTree](https://demos.marilo.com/blazor-ui/dropdowntree/overview)
