...
The Editor tools and commands are in the `Marilo.Blazor.Components.Editor` namespace. If you will be applying settings related to the tools (such as choosing a different built-in toolset or adding custom tools), you should add an `using` statement for this namespace.
...
To add a button, add a `new <Command>()` where `Command` is the name of the tool from [this table](slug:editor-built-in-tools#built-in-tools-and-commands). The VS Intellisense can also show you the classes in the `Marilo.Blazor.Components.Editor.` namespace.
