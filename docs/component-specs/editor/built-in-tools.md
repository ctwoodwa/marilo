...
tags: marilo,blazor,built-in,tools
published: True
position: 40
components: ["editor"]
---
...
* `Tool Type` - a tool can be a [button](slug:Marilo.Blazor.Components.Editor.ButtonTool), a [dropdown list](slug:Marilo.Blazor.Components.Editor.DropDownListTool) or a [color picker](slug:Marilo.Blazor.Components.Editor.ColorTool). Each of these three types exposes some customization options. See the examples for the [color tools](#color-tool-customization), [font tools](#font-tool-customization) and the [Format tool](#format-tool-customization). Only *buttons* can be added to tool groups in the [toolbar](slug:editor-toolbar).
...
@using EditorNS = Marilo.Blazor.Components.Editor;

<MariloButton OnClick="@InsertParagraph">Insert Paragraph in the Editor</MariloButton>

<MariloEditor @ref="EditorRef"
               Tools="@EditorTools"
               @bind-Value="@EditorValue">
</MariloEditor>

@code {
    private MariloEditor EditorRef { get; set; }
...
| --- | --- | --- |
| `Colors` | `IEnumerable<string>`. | The list of available colors to set from the Color tool. You can provide a member of [`ColorPalettePresets`](slug:Marilo.Blazor.ColorPalettePresets), or a custom list of [RGB(A) or HEX colors in different supported formats](slug:colorpicker-overview#supported-value-formats). |
...
<MariloEditor Tools="@EditorTools"
               @bind-Value="@EditorValue">
</MariloEditor>
...
The [`FontFamily`](slug:Marilo.Blazor.Components.Editor.FontFamily) and [`FontSize`](slug:Marilo.Blazor.Components.Editor.FontSize) tools have a `Data` property that accepts a `List<EditorDropDownListItem>`. Use it to customize the available options in these dropdowns. You can also change the dropdown label via `DefaultText`.
...
The [`Format` tool exposes a `Data` property](slug:Marilo.Blazor.Components.Editor.Format) that accepts a `List<EditorDropDownListItem>`. Use it to reduce or reorder the items in the dropdown list.
...
<MariloButton OnClick="@BoldText">Create bold text</MariloButton>
<MariloButton OnClick="@InsertTable">Insert Table</MariloButton>
<MariloButton OnClick="@InsertImage">Insert Image</MariloButton>
<MariloButton OnClick="@InsertInlineText">Insert Inline Text</MariloButton>
...
@code{
    ...
    {
        await TheEditor.ExecuteAsync(new ImageCommandArgs("https://demos.marilo.com/blazor-ui/images/articles/1-220x140.png", "the alt text", "220px", "140px"));
    }
