...
description: Expand Items in the Marilo PanelBar.
slug: panelbar-expand-items
tags: marilo,blazor,panelbar,expand,items
published: True
position: 5
components: ["panelbar"]
---
...
You can expand a single or multiple items in the Marilo PanelBar.
...
You can use the ExpandMode to allow the user to expand one or more items at a time. To control that set the `ExpandMode` parameter of the `<MariloPanelBar>` to a member of the `PanelBarExpandMode` enum:
...
<MariloButton OnClick="@ExpandItems">Expand First and Second Item</MariloButton>
<MariloButton OnClick="@CollapseItems">Collapse the Expanded Items</MariloButton>
