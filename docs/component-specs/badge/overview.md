...
slug: badge-overview
tags: marilo,blazor,badge,navbar
published: True
position: 0
components: ["badge"]
---
...
The <a href = "https://www.marilo.com/blazor-ui/badge" target="_blank">Blazor Badge component</a> is a visual element that displays additional information, such as a status indicator, notification icon, short text, and others.
...
1. Add the `<MariloBadge>` tag to a container that the Badge will attach to.
1. Use the Badge `ChildContent` to add content.
1. (optional) Customize the [appearance](slug:badge-appearance) and fine-tune the [positioning and alignment](slug:badge-position-alignment) of the Marilo Blazor Badge.

>caption Basic configuration of the Blazor Badge

````RAZOR
<MariloButton>
    <MariloBadge VerticalAlign="@BadgeVerticalAlign.Bottom">12</MariloBadge>
    Notifications
</MariloButton>
````
...
| `FillMode` | `Marilo.Blazor.ThemeConstants.Badge.FillMode.Solid` | Controls how the Badge is filled. |
| `Rounded` | `Marilo.Blazor.ThemeConstants.Badge.Rounded.Full` | Defines how rounded the borders of the Badge are. |
| `Size` | `Marilo.Blazor.ThemeConstants.Badge.Size.Medium` | Sets the size of the Badge. |
| `ThemeColor` | `Marilo.Blazor.ThemeConstants.Badge.ThemeColor.Primary` | Adjusts the background color of the Badge. |
...
* [Live Badge Demos](https://demos.marilo.com/blazor-ui/badge/overview)
* [Badge API Reference](slug:Marilo.Blazor.Components.MariloBadge)
