...
tags: marilo,blazor,textbox,overview
published: True
position: 0
components: ["textbox"]
---
...
The <a href="https://www.marilo.com/blazor-ui/textbox" target="_blank">Blazor TextBox component</a> allows the user to enter a generic plain text message. 
...
1. Add the `<MariloTextBox>` tag to a Razor file.
...
<MariloTextBox @bind-Value="@StringValue" />
...
| `Password` | `bool` | When set to `true`, the HTML element renders `type="password"` so that the user input is hidden. You can find examples of validation and reveal buttons in the [Password Textbox demo](https://demos.marilo.com/blazor-ui/textbox/password) and the [Add Eye Icon to Reveal a TextBox Password](slug:textbox-kb-eye-reveal-password) article. |
...
<MariloTextBox @ref="@TextBoxRef"
                @bind-Value="@TextBoxValue"
                Width="200px" />

@code {
    private MariloTextBox TextBoxRef { get; set; }
...
<MariloTextBox Placeholder="john@smith.com" Title="write your email here"
...
<MariloButton OnClick="@ChangeValue">Change TextBox Value</MariloButton>
...
* [Live Demo: TextBox](https://demos.marilo.com/blazor-ui/textbox/overview)
* [TextBox API Reference](slug:Marilo.Blazor.Components.MariloTextBox)
