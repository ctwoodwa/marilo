...
tags: marilo,blazor,treeview,drap,drop,dragdrop,overview
published: True
position: 11
components: ["treeview"]
---
...
The Drag and Drop functionality for the TreeView allows you to move a node or multitude of nodes between different parents in the same treeview or between different Marilo TreeView instances.
...
1. Set the `Draggable` parameter of the `<MariloTreeView>` to `true`
...
The `OnDrop` event fires when the user drops a node to a new location. It is triggered only if the new location is a Marilo component. The event allows you to manipulate your data collection based on where the user dropped the element. 
...
The `OnDragEnd` event fires when a drag operation ends. The event is triggered after `OnDrop` and unlike it, `OnDragEnd` will fire even if the drop location is not a Marilo component. In this case, the non-applicable event arguments will be `null`.
...
@code {
    ...
    private MariloGrid<Person> GridRef { get; set; }

    private MariloTreeView TreeRef { get; set; }
...
using System.Linq;
using Marilo.Blazor;
using MariloBlazorAppSource.Models;
...
<MariloTreeView Data="@HierarchicalData"
                 Draggable="@Draggable"
                 OnDrop="@OnDrop">
    <TreeViewBindings>
...
@code {
    public MariloTreeView FirstTree { get; set; }
    public MariloTreeView SecondTree { get; set; }
...
* [Live Demo: TreeView Drag and Drop](https://demos.marilo.com/blazor-ui/treeview/drag-drop)
* [Data Binding a TreeView](slug:components/treeview/data-binding/overview)
* [TreeView API Reference](slug:Marilo.Blazor.Components.MariloTreeView)
