...
tags: marilo,blazor,treeview,checkbox,checkboxes,nodes,multiple
published: True
position: 10
components: ["treeview"]
---
...
To let the user use **multiple** node selection, set the `CheckBoxMode` parameter to `Marilo.Blazor.TreeViewCheckBoxMode.Multiple`.
    ...
    <ul>
        @if (checkedItems.Any())
        {
            foreach (var item in checkedItems)
            {
                ...
                <li>
                    <span>
                        <MariloSvgIcon Icon="@checkedItem.Icon"></MariloSvgIcon>
                    </span>
                    <span>
                        @(checkedItem.Text)
                    </span>
