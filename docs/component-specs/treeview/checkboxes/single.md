...
tags: marilo,blazor,treeview,checkbox,checkboxes,node,single
published: True
position: 5
components: ["treeview"]
---
...
To let the user check only a **single** node in the TreeView, set the `CheckBoxMode` parameter to `Marilo.Blazor.Components.TreeViewCheckBoxMode.Single`.
