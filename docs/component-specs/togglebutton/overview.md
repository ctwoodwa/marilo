...
tags: marilo,blazor,Toggle,button,overview
published: True
position: 0
...
This article provides information about the <a href="https://www.marilo.com/blazor-ui/togglebutton" target="_blank">Blazor ToggleButton component</a> and its core features.

The ToggleButton component can have a selected state, which is the main difference from the [regular Marilo UI for Blazor Button](slug:components/button/overview). The two-state styling depends on the [chosen theme](slug:themes-overview). The ToggleButton also provides [events](slug:togglebutton-events), [declarative appearance customization](slug:togglebutton-appearance) and can nest [icons](slug:togglebutton-icons).

## Creating Blazor ToggleButton

1. Use the `<MariloToggleButton>` tag.
...
>caption Basic Marilo ToggleButton

````RAZOR
<MariloToggleButton @bind-Selected="@IsSelected"
...
The Toggle Button provides a built-in way to render [Marilo font icons, custom font icons, images or sprites](slug:togglebutton-icons).
...
````RAZOR
<MariloToggleButton Class="my-toggle">
    Toggle Button
</MariloToggleButton>
...
* [Live Demo: ToggleButton](https://demos.marilo.com/blazor-ui/togglebutton/overview)
* [ToggleButton API Reference](slug:Marilo.Blazor.Components.MariloToggleButton)
