...
tags: marilo,blazor,wizard,buttons
published: True
position: 5
components: ["wizard"]
---
...
This article provides details on the buttons available in the Marilo Wizard.
...
The Wizard component allows you to include your custom buttons. You can define the desired buttons under the `WizardButtons` tag of the `MariloWizard`.
...
<div style="text-align:center">
    <MariloWizard  @bind-Value="@Value" Width="600px" Height="300px">
        <WizardSteps>
            <WizardStep Text="1">
                <Content>
                    <h2>Content for Wizard Step 1</h2>
        ...
        <WizardButtons>
            @{
                ...
                if (index > 0)
                {
                    <MariloButton OnClick="@(() => Value = 0)">Go to first page</MariloButton>
                    <MariloButton OnClick="@(() => Value -= 1)">Previous</MariloButton>
                }
                if (index != 2)
                {
                    <MariloButton ButtonType="ButtonType.Button" ThemeColor="primary" OnClick="@(() => Value += 1)">Next</MariloButton>
                    <MariloButton ButtonType="ButtonType.Button" ThemeColor="primary" OnClick="@(() => Value = 2)">Go to last page</MariloButton>
                }
            }
        </WizardButtons>
    </MariloWizard>
...
````RAZOR
<MariloWizard Class="disabled-stepper" />
...
````RAZOR
<MariloWizard Class="hidden-stepper" />
...
@if (ShowWizard)
{
    <MariloWizard @bind-Value="@CurrentWizardStep" OnFinish="@OnWizardFinish">
        <WizardSettings>
        ...
        <WizardButtons>
            @{
                ...
                {
                    <MariloButton OnClick="@( () => PreviousClick(currentStepIndex) )">Back</MariloButton>
                }
                if (currentStepIndex < 2)
                {
                    <MariloButton ThemeColor="primary" OnClick="@( () => NextClick(currentStepIndex) )">Next</MariloButton>
                }
                else
                {

                    <MariloButton ThemeColor="primary" OnClick="@DoneClick">Done</MariloButton>
                }
            }
        </WizardButtons>
    </MariloWizard>
...
* [Live Demos: Wizard Buttons](https://demos.marilo.com/blazor-ui/wizard/buttons)
