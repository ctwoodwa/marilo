...
tags: marilo,blazor,wizard,form,integration
published: True
position: 5
components: ["wizard"]
---
...
The Wizard component provides integration with the [Marilo Form](slug:form-overview).

Each step of the Wizard can include an instance of Marilo Form inside its `Content` tag. You will be able to use all the built-in features of the Form including its [Validation](slug:form-validation) to achieve the desired Wizard configuration.
...
>caption Integrate a Form component in the Marilo Wizard. Use its validation state to control the `Valid` parameter of the Steps. The result from the snippet.
    ...
    <MariloWizard @bind-Value="@Value" OnFinish="@OnFinishHandler" Width="700px">
        <WizardSteps>
                        ...
                        <FormItems>
                            ...
                            <FormItem Field="@nameof(User.Email)">
                                <Template>
                                    ...
                                    <MariloTextBox Id="mail" @bind-Value="@UserModel.Email" InputMode="email" PlaceHolder="example@domain.com"></MariloTextBox>
                                    <MariloValidationMessage For="@(() => UserModel.Email)"></MariloValidationMessage>
                                </Template>
                            </FormItem>
                            <FormItem Field="@nameof(User.Password)">
                                <Template>
                                    <label for="pass" class="k-label k-form-label">Password*:</label>
                                    <MariloTextBox Id="pass" @bind-Value="@UserModel.Password" Password="true"></MariloTextBox>
                                    <MariloValidationMessage For="@(() => UserModel.Password)"></MariloValidationMessage>
                                </Template>
                            </FormItem>
            ...
            <WizardStep Label="Shipping address" Icon="@SvgIcon.MapMarkerTarget" OnChange="@OnShippingStepChange" Valid="@IsShippingValid">
                <Content>
                    <MariloForm Model="@ShippingModel"
                                 @ref="ShippingForm">
                        <FormValidation>
                            <DataAnnotationsValidator></DataAnnotationsValidator>
                        </FormValidation>
...
@code {
    ...
    public MariloForm RegisterForm { get; set; }
    public User UserModel { get; set; } = new User();

    public MariloForm ShippingForm { get; set; }
...
## See Also
  ...
  * [Live Demos: Wizard Form](https://demos.marilo.com/blazor-ui/wizard/form)