...
description: Discover the Marilo UI for Blazor DropZone, learn how to create the component, integrate it with the Upload and FileSelect controls, use its templaes, parameters, style its appearance, and explore its examples.
slug: dropzone-overview
tags: marilo,blazor,dropzone,external,drag,drop,file,overview
published: True
position: 0
components: ["dropzone"]
---
...
The <a href="https://www.marilo.com/blazor-ui/dropzone" target="_blank">Marilo UI for Blazor DropZone component</a> allows you to declare an external drop zone for an existing [FileSelect](slug:fileselect-overview) or [Upload](slug:upload-overview) component.
...
1. Add the DropZone to your Razor page by using the `MariloDropZone` tag.
...
````RAZOR
<MariloDropZone Id="@DropZoneId" NoteText="@NoteText" />

<MariloFileSelect DropZoneId="@DropZoneId" AllowedExtensions="@AllowedExtensions" />
...
| `DragOverClass` | `string` | The CSS class that will be rendered on the main wrapping element of the DropZone when a file is dragged over it. Use it for [conditionally styling the component during a dragover action](https://demos.marilo.com/blazor-ui/dropzone/overview). |
...
* [Live Demo: Blazor DropZone Overview and Key Features](https://demos.marilo.com/blazor-ui/dropzone/overview)
