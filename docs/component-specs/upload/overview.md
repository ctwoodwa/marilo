...
tags: marilo,blazor,upload,async,overview
published: True
position: 0
components: ["upload"]
---
...
````CS
<MariloUpload SaveUrl="/api/upload/save"
...
The `UploadController` class below assumes that the project name and namespace is `MariloBlazorApp`.
...
namespace MariloBlazorApp.Controllers
{
...
The Marilo Upload component makes XHR requests from the browser to the designated endpoints. If needed, use the [`OnUpload` and `OnRemove` events](slug:upload-events) to add headers, authentication tokens and custom data to the request.
...
You can use the functionality of the built-in template and modify the appearance of the **Select files...** button. [Read more about the Marilo Upload templates...](slug:upload-templates)
...
Additionally, you can define an external drop zone by using the [Marilo UI for Blazor DropZone component](slug:dropzone-overview).
...
````RAZOR
<p>
    <MariloButton OnClick="@SelectFiles">Open File Selection Dialog</MariloButton>
    <MariloButton OnClick="@Clear">Clear File List</MariloButton>
    <MariloButton OnClick="@Upload">Start Upload</MariloButton>
</p>

<MariloUpload @ref="@UploadRef"
               SaveUrl="/api/upload/save"
               RemoveUrl="/api/upload/remove"
               AutoUpload="false" />

@code {
    private MariloUpload UploadRef { get; set; }
...
* [Live Upload Demos](https://demos.marilo.com/blazor-ui/upload/overview)
* [Upload API](slug:Marilo.Blazor.Components.MariloUpload)
