...
tags: marilo,blazor,chart,rendering,modes
published: True
position: 5
components: ["charts"]
---
...
The Chart for Blazor supports two modes for rendering its data. You can set the desired rendering mode through the `RenderAs` parameter the `MariloChart` exposes. It takes a member of the `Marilo.Blazor.RenderingMode` enum and depending on the result you are trying to achieve, you can choose one of the following:
