...
tags: marilo,blazor,chart,radar,area
published: True
position: 0
components: ["charts"]
---
...
If some values are missing from the series data (they are `null`), you can have the chart work around this by setting the `MissingValues` property of the series to the desired behavior (member of the `Marilo.Blazor.ChartSeriesMissingValues` enum):
...
You can render the lines between the points with different styles. The supported styles can be set via the `Style` property of the child `ChartSeriesLine` tag - it takes a member of `Marilo.Blazor.ChartSeriesLineStyle` enum:
...
## See Also
  ...
  * [Live Demo: Radar Area Chart](https://demos.marilo.com/blazor-ui/chart/radar-area-chart)
