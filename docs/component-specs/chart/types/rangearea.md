...
tags: marilo,blazor,chart,rangearea
published: True
position: 0
components: ["charts"]
---
...
If both values in a range pair are missing or `null`, you can have the Chart work around this by setting the `MissingValues` property of the series to the desired behavior. Use a member of the `Marilo.Blazor.ChartSeriesMissingValues` enum:
...
You can render the lines between the points with different styles. The supported styles can be set via the `Style` property of the child `ChartSeriesLine` tag—it takes a member of the `Marilo.Blazor.ChartSeriesLineStyle` enum:
...
* [Live Demo: Range Area Chart](https://demos.marilo.com/blazor-ui/chart/range-area-chart)
