...
tags: marilo,blazor,inlineaiprompt,events
published: true
position: 10
...
This article describes the events of the Marilo InlineAIPrompt for Blazor:
...
To define the available output actions, set the `OutputActions` parameter to a list of [`InlineAIPromptOutputActionDescriptor`](slug:Marilo.Blazor.Components.InlineAIPromptOutputActionDescriptor) objects. Each action descriptor configures the appearance and behavior of an action button.

The event handler receives an argument of type [`InlineAIPromptOutputActionClickEventArgs` API reference](slug:Marilo.Blazor.Components.InlineAIPromptOutputActionClickEventArgs), which provides details about the clicked action, the prompt, the output, and the related command (if any).
...
The event handler receives an argument of type [`InlineAIPromptPromptRequestEventArgs` API reference](slug:Marilo.Blazor.Components.InlineAIPromptPromptRequestEventArgs). See the [example below](#example).
...
The event handler receives an argument of type [`InlineAIPromptCommandExecuteEventArgs` API reference](slug:Marilo.Blazor.Components.InlineAIPromptCommandExecuteEventArgs). See the [example below](#example).
...
<MariloInlineAIPrompt @ref="@InlinePromptRef"
...
@code {
    ...
    private MariloInlineAIPrompt? InlinePromptRef { get; set; }
