...
slug: gantt-state
tags: marilo,blazor,gantt,state,save,load,layout,get,set,change
published: True
position: 15
components: ["gantt"]
---
...
@code {
    private MariloGantt<GanttTask> GanttRef;
    ...
    private async Task OnStateInitHandler(GanttStateEventArgs<GanttTask> args)
    {
        ...
        var state = new GanttState<GanttTask>
            {
                SortDescriptors = new List<Marilo.DataSource.SortDescriptor>
                {
                new Marilo.DataSource.SortDescriptor{ Member = "End", SortDirection = Marilo.DataSource.ListSortDirection.Ascending }
                },
...
<MariloButton OnClick="@ReloadPage">Reload the page to see the current grid state preserved</MariloButton>
<MariloButton OnClick="@ResetState">Reset the state</MariloButton>
...
<MariloButton OnClick="@StartInsert">Insert at Root Level</MariloButton>
<MariloButton OnClick="@EditTaskOne">Edit First Task</MariloButton>
<MariloButton OnClick="@ExitEditMode">Exit Edit Mode</MariloButton>
...
<MariloButton OnClick="@GetCurrentColumnsState">Get Column State</MariloButton>
...
* [Live Demo: Gantt State](https://demos.marilo.com/blazor-ui/gantt/persist-state)
