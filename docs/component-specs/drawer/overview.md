...
tags: marilo,blazor,drawer,overview
published: True
position: 0
components: ["drawer"]
---
...
The <a href="https://www.marilo.com/blazor-ui/drawer" target="_blank">Blazor Drawer component</a> is an interactive side panel for [navigating](slug:drawer-navigation) in responsive web applications. It can be always visible, or expanded and collapsed. The Drawer allows switching the content of different sections on the page. You can customize its [templates](slug:drawer-templates), [expand modes](slug:drawer-modes), [minimize behavior](slug:drawer-mini-mode) and also respond to [events](slug:drawer-events).

## Creating Blazor Drawer

1. Add the `MariloDrawer` tag to a Razor file.
...
<MariloButton OnClick="@(() => DrawerRef.ToggleAsync())"
               Icon="@SvgIcon.Menu">
    Toggle drawer
</MariloButton>

<MariloDrawer Data="@Data" Mode="@DrawerMode.Push"
               @ref="@DrawerRef">
    <DrawerContent>lorem ipsum</DrawerContent>
</MariloDrawer>

@code {
    Marilo.Blazor.Components.MariloDrawer<DrawerItem> DrawerRef { get; set; }
...
* [Drawer API Reference](slug:Marilo.Blazor.Components.MariloDrawer-1)
