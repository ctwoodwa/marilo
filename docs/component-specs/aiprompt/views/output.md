...
tags: marilo,blazor,aiprompt,ai,prompt,output
published: True
position: 20
components: ["aiprompt"]
---
...
By default, the Output view is rendered and is part of the predefined views. However, if you provide a render fragment of type `AIPromptViews` to the `MariloAIPrompt` tag, you override the default rendering, meaning you must explicitly add `AIPromptOutputView` tag within it.

>caption Use the `ButtonText` and `ButtonIcon` to alter the appearance of view button.

````RAZOR
<MariloAIPrompt @bind-Prompt="@Prompt">
    <AIPromptViews>
