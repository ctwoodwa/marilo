...
tags: marilo,blazor,aiprompt,ai,prompt
published: True
position: 0
components: ["aiprompt"]
---
...
The <a href = "https://www.marilo.com/blazor-ui/ai-prompt" target="_blank">Blazor AIPrompt component</a> helps you write input (prompt) instructing the Generative AI to produce the desired response. 
...
>caption Basic configuration of the Marilo AIPrompt

````RAZOR
<MariloAIPrompt OnPromptRequest="@HandlePromptRequest"
...
For a complete list of available parameters and methods, refer to the [AIPrompt API Reference](slug:Marilo.Blazor.Components.MariloAIPrompt).

To use component methods, define a reference to the AIPrompt instance with the `@ref` directive. For example:

````RAZOR
<MariloAIPrompt @ref="@AIPromptRef" 
                 OnPromptRequest="@HandlePromptRequest"
                 PromptSuggestions="@Suggestions">
</MariloAIPrompt>
<div style="margin-top: 2em;">
    <MariloTextBox @bind-Value="@CustomPrompt"></MariloTextBox>
    <MariloButton OnClick="@ExternalGenerateHandler">Generate</MariloButton>
</div>

@code {
    private string CustomPrompt { get; set; }
    private MariloAIPrompt AIPromptRef { get; set; }
...
To integrate a built-in SpeechToTextButton in the AIPrompt component, set `EnableSpeechToText="true"`. Optionally, you can use the `<AIPromptSettings>` tag as a child of `<MariloAIPrompt>`. Inside `<AIPromptSettings>`, you can define the `<AIPromptSpeechToTextButtonSettings>` tag to configure the appearance and behavior of the built-in SpeechToTextButton. For a complete list of available parameters, refer to the [AIPromptSpeechToTextButtonSettings API Reference](slug:Marilo.Blazor.Components.AIPromptSpeechToTextButtonSettings).
...
````RAZOR.skip-repl
<MariloAIPrompt EnableSpeechToText="true" Prompt="@Prompt" PromptChanged="@OnPromptChanged">
    <AIPromptSettings>
...
* [Live Demo: AIPrompt](https://demos.marilo.com/blazor-ui/aiprompt/overview)
* [AIPrompt API Reference](slug:Marilo.Blazor.Components.MariloAIPrompt)
