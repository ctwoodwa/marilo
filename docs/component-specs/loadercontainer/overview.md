...
tags: marilo,blazor,loader,container,loadercontainer,overview
published: True
position: 0
...
The <a href = "https://www.marilo.com/blazor-ui/loader-container" target="_blank">Blazor LoaderContainer</a> provides an animated indicator, a panel, and an overlay that can be used when the application is performing a time-consuming background operation, for example loading data.
...
1. Use the `<MariloLoaderContainer>` tag.
...
<MariloLoaderContainer Visible="@( Data == null )" Text="Please wait..." />
...
| `Size` | `string`<br />(`"md"`) | Sets the [size of the animated graphic](slug:loadercontainer-appearance#size). For convenience, use the members of the static class [`ThemeConstants.Loader.Size`](slug:Marilo.Blazor.ThemeConstants.Loader.Size). |
...
| `ThemeColor` | `string`<br />(`"primary"`) | Sets the [color of the animated graphic and text](slug:loadercontainer-appearance#themecolor). For convenience, use the members of the static class [`ThemeConstants.Loader.ThemeColor`](slug:Marilo.Blazor.ThemeConstants.Loader.ThemeColor). |
...
>
> You can find one example in the following **sample project**: <a href="https://github.com/marilo/blazor-ui/tree/master/loader/block-all-content" target="_blank">Disable All Content with Marilo Loader</a>
...
<MariloLoaderContainer Class="no-panel"
...
* [LoaderContainer API Reference](slug:Marilo.Blazor.Components.MariloLoaderContainer)
