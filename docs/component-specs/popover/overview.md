...
tags: marilo,blazor,popover,pop,over
published: True
position: 0
components: ["popover"]
---
...
>caption Basic Marilo Popover for Blazor

````RAZOR
<MariloPopover @ref="@PopoverRef"
                AnchorSelector=".popover-target"
                ShowOn="@PopoverShowOn.Click"
                Position="@PopoverPosition.Bottom"
                Offset="20">
    <PopoverContent>
        Marilo Popover for Blazor
    </PopoverContent>
    <PopoverActions>
        <MariloButton OnClick="@( () => PopoverRef?.Hide() )"
                       Icon="@SvgIcon.X">Close</MariloButton>
    </PopoverActions>
</MariloPopover>

<MariloButton Class="popover-target">Show Popover Automatically</MariloButton>

<MariloButton OnClick="@( () => PopoverRef?.Show() )">Show Popover Programmatically</MariloButton>

@code{
    private MariloPopover? PopoverRef { get; set; }
}
````
...
| Method  | Description |
|---------|-------------|
| `Refresh` | Use this method to programmatically re-render the Popover. <br /> The Popover is rendered as a child of the `MariloRootComponent`, instead of where it is declared. As a result, it doesn't automatically refresh when its content is updated. In such cases, the `Refresh` method comes in handy to ensure that the Popover content is up-to-date. |
...
<MariloPopover @ref="@PopoverRef"
    ...
    <PopoverActions>
        <MariloButton OnClick="@( () => PopoverRef?.Hide() )"
                       Icon="@SvgIcon.X">Hide</MariloButton>
    </PopoverActions>
</MariloPopover>

<MariloButton OnClick="@( () => PopoverRef?.Show() )">Show Popover</MariloButton>

<MariloSvgIcon Class="popover-target" Icon="@SvgIcon.QuestionCircle" />
...
* [Live Popover Demos](https://demos.marilo.com/blazor-ui/popover/overview)
* [Popover API Reference](slug:Marilo.Blazor.Components.MariloPopover)
