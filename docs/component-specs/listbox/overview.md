...
@code {
    private MariloListBox<ListBoxModel> ListBoxRef { get; set; } = null!;
...
````RAZOR
<MariloListBox Data="@ListBoxStrings"
...
| `Size` | `string` <br /> (`"md"`) | This parameter controls ListBox styles such as paddings or font size. For easier usage, set the parameter to a member of the static class `Marilo.Blazor.ThemeConstants.ListBox.Size`. |
...
<MariloButton OnClick="@AddListBoxItem">Add ListBox Item</MariloButton>
...
* [ListBox API Reference](slug:Marilo.Blazor.Components.MariloListBox-1)
