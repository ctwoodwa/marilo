...
<MariloListBox @ref="@ListBoxRef"
    ...
    <ItemTemplate>
        <MariloSvgIcon Icon="@context.Icon" />
        @context.FirstName <strong>@context.LastName</strong>
    </ItemTemplate>
    <NoDataTemplate>
        <div>
            No items?..<br />
            <MariloButton OnClick="@AddItems">Add Some</MariloButton>
        </div>
    </NoDataTemplate>
</MariloListBox>

@code {
    private MariloListBox<Person> ListBoxRef { get; set; } = null!;
...
* [Live Demo: ListBox Templates](https://demos.marilo.com/blazor-ui/listbox/templates)
