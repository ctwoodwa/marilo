...
slug: card-actions
tags: marilo,blazor,card,actions
published: True
position: 9
components: ["card"]
---
...
Marilo Button components with the `k-flat` class are designed to fit in the card design as actions, so you can use them as your first choice of action components. You can find examples with them below.
...
You can define the orientation of the buttons through the `Orientation` parameter of the `CardActions`. It takes a member of the `Marilo.Blazor.CardOrientation` enum:
...
The `Layout` parameter takes a member of the `Marilo.Blazor.CardActionsLayout` enum:
* `Center` - the default
* `End`
* `Start`
* `Stretch`
...
<MariloCard Width="600px">
    ...
    <CardActions Layout="@CardActionsLayout.Stretch">
        <MariloButton Class="k-flat">Action 1</MariloButton>
        <MariloButton Class="k-flat">Action 2</MariloButton>
        <MariloButton Class="k-flat">Action 3</MariloButton>
    </CardActions>
...
## See Also
  ...
  * [Live Demo: Card](https://demos.marilo.com/blazor-ui/card/actions)
