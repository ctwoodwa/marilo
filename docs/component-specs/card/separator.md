...
slug: card-separator
tags: marilo,blazor,card,separator
published: True
position: 5
components: ["card"]
---
...
* `Orientation` - `CardOrientation` - defines the orientation of the `CardSeparator`. Takes a member of the `Marilo.Blazor.CardOrientation` enum (`Horizontal` or `Vertical`).
