...
description: Built-in Tools in the Marilo ToolBar for Blazor
slug: toolbar-built-in-tools
tags: marilo,blazor,toolbar,builtin,tools,button,buttons
published: True
position: 1
components: ["toolbar"]
---
...
The Marilo ToolBar for Blazor allows you to use built-in buttons and button groups or to add a [custom tool](slug:toolbar-templated-item). The available built-in tools are:
...
You can add multiple buttons to the Marilo Toolbar. To do that you should add the `<ToolBarButton>` to the `<MariloToolBar>`.
...
| `Icon` | `object` | Adds a font icon to the button. You can find more information on adding a font icon to a Marilo Component in [Marilo Font and Svg Icons article](slug:common-features-icons#icon-namespaces). |
...
>caption The Marilo ToolBar with ToolBarButtons

![Blazor Toolbar Toolbarbutton Example](images/toolbar-toolbarbutton-example.png)

````RAZOR
@*This example shows the MariloToolBar with ToolBarButtons and their features*@
...
You can add multiple toggle  buttons to the Marilo Toolbar. To do that you should add the `<ToolBarToggleButton>` to the `<MariloToolBar>`.
...
>caption The Marilo ToolBar with ToolBarToggleButtons

![Blazor Toolbar Togglebutton Example](images/toolbar-togglebutton-example.png)

````RAZOR
@*This example shows the MariloToolBar with ToolBarToggleButton and its available features*@
...
You can add one or more group of buttons to the Toolbar. To do that you should add the `<ToolBarButtonGroup>` to the `<MariloToolBar>`. In the button group you can place either the `ToolBarButton` or the `ToolBarToggleButton`.
...
>caption The Marilo ToolBar with grouped buttons

![Blazor Toolbar Grouped Buttons](images/toolbar-grouped-buttons.png)

````RAZOR
@*This example shows the MariloToolBar with grouped ToolBarButtons*@
...
## See Also
  ...
  * [Live Demo: ToolBar Overview](https://demos.marilo.com/blazor-ui/toolbar/overview)
