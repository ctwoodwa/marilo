...
description: Templated Item in the Marilo ToolBar for Blazor
slug: toolbar-templated-item
tags: marilo,blazor,toolbar,template,item,templateditem
published: True
position: 10
components: ["toolbar"]
---
...
The Marilo ToolBar for Blazor allows you to add a custom element. To use it, add the `<ToolBarTemplateItem>` inside the `<MariloToolBar>`. In it, you can add HTML and components according to your application needs.
...
>caption Add a Marilo DropDownList to the ToolBar

![templated item for the ToolBar](images/toolbar-templated-item.png)

````RAZOR
@*Use the templated item to add a Marilo DropDownList to the ToolBar*@
...
<MariloToolBar>
    ...
    <ToolBarTemplateItem>
        <MariloDropDownList Data="@Roles" Value="@SelectedRole" ValueChanged="@( (string v) => RoleChange(v) )"></MariloDropDownList>
    </ToolBarTemplateItem>
</MariloToolBar>
