...
description: Try now the Marilo UI for Blazor Context Menu and learn more about its options for creating the component, binding it to data, changing its navigation views, customizing the component per target, and more.
slug: contextmenu-overview
tags: marilo,blazor,context menu,overview
published: True
position: 0
...
The <a href="https://www.marilo.com/blazor-ui/context-menu" target="_blank">Blazor Context Menu</a> displays a contextual popup with flat or hierarchical data in a traditional menu-like structure. The component enables you to invoke commands while preserving the screen real estate.
...
1. Add the `MariloContextMenu` tag and set its `Selector` parameter to a CSS selector that will match the elements to which you want to attach the Context Menu.
...
<MariloContextMenu Selector=".context-menu-target" Data="@MenuItems"                       
...
The following table lists Context Menu parameters, which are not related to other features on this page. For the full list of properties, methods, and events, see the [Context Menu API reference documentation](slug:Marilo.Blazor.Components.MariloContextMenu-1).
...
<MariloContextMenu Data="@MenuItems" @ref="@TheContextMenu">
    <Template>
        @{
        ...
        <MariloTextBox @bind-Value="@TextBoxValue" />
        <br />
        <MariloButton OnClick="@HandleTextBoxReset">Reset textbox</MariloButton>
        <MariloButton OnClick="@(async () => await TheContextMenu.HideAsync())">Close</MariloButton>
    </Template>
</MariloContextMenu>
...
@code {
    ...
    MariloContextMenu<ContextMenuItem> TheContextMenu { get; set; }
...
* [Binding the Marilo UI for Blazor Context Menu to Data](slug:contextmenu-data-binding-overview)
...
* [Live Demo: Overview of the Blazor Context Menu](https://demos.marilo.com/blazor-ui/contextmenu/overview)
* [Context Menu API Reference](slug:Marilo.Blazor.Components.MariloContextMenu-1)
