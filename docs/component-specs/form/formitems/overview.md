...
slug: form-formitems
tags: marilo,blazor,form,edit,formitems
published: True
position: 0
components: ["form"]
---
...
Each custom editor is expressed through the `FormItem` tag. You can define a collection of those editors in the `FormItems` tag that is a child of the `MariloForm` tag. You can find some [code examples](#example) below.
...
* Use a Marilo input component inside a [Form item `<Template>`](slug:form-formitems-template). Define two-way binding for the `Value` parameter, or use `Value`, `ValueChanged`, and `ValueExpression`.
        ...
        <FormItem Field="@nameof(Person.Name)">
            <Template>
                <label class="k-label k-form-label" style="color: var(--kendo-color-success)">
                    Marilo component - re-render inside and outside the Form.
                </label>
                <div class="k-form-field-wrap">
                    <MariloTextBox @bind-Value="@Employee.Name" DebounceDelay="0" />
                    <MariloValidationMessage For="@( () => Employee.Name )" />
                </div>
            </Template>
        </FormItem>
        <FormItem Field="@nameof(Person.Name)">
...
@code {
    ...
    // See https://www.marilo.com/blazor-ui/documentation/knowledge-base/inputs-validation-child-component
    [Parameter]
...
* [Online demo: Form Items](https://demos.marilo.com/blazor-ui/form/overview)
