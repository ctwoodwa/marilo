...
tags: marilo,blazor,form,layout
published: True
position: 6
components: ["form"]
---
...
>tip The content on this page applies to Marilo UI for Blazor version **4.2.0** and above.
...
| Feature | Description |
| --- | --- |
| [`FormItemsTemplate`](#formitemstemplate) | A `RenderFragment` parameter of the `MariloForm` component, so it's usually used as a child tag. |
| [`<MariloFormGroupRenderer>`](#form-group-renderer) | A component that renders a specific *Form group*. It can occur anywhere inside `<FormItemsTemplate>`. |
| [`<MariloFormItemRenderer>`](#form-item-renderer) | A component that renders a specific *Form item*. It can occur inside `<FormItemsTemplate>` or the `Template` of a `<MariloFormGroupRenderer>`. |
...
* [`<MariloFormGroupRenderer>`](#form-group-renderer) defines the **rendering place** of a specific form **group**.
* [`<MariloFormItemRenderer>`](#form-item-renderer) defines the **rendering place** of a specific form **item**.
...
Be aware of [nested form contexts](#nested-form-contexts) when using `MariloFormGroupRenderer`.


## Form Group Renderer

The `MariloFormGroupRenderer` component defines the rendering place of a Form group inside the `FormItemsTemplate`. The component has the following parameters:
...
The `MariloFormItemRenderer` component defines the rendering place of a Form item inside the `FormItemsTemplate`. The component has the following parameters:
...
>tip `MariloFormItemRenderer` is unrelated to the [template of a single `<FormItem>`](slug:form-formitems-template). The two features are used separately and independently.
...
1. Inside `<FormItemsTemplate>`, define a `<MariloFormGroupRenderer>` and `<MariloFormItemRenderer>` for each group and item that exists in `<FormItems>`. Place each Renderer tag at the desired place. This steps also applies to `<FormAutoGeneratedItems />`, which need Renderers too.
1. If using conditional statements to render `<FormItem>` tags, then define these tags unconditionally. Instead, use conditional logic for `<MariloFormItemRenderer>` tags inside `<FormItemsTemplate>`.
1. If [using `<FormItem>` instances inside a nested Razor component](slug:form-kb-use-items-in-custom-component), then see the linked KB article and:
    ...
    * Use `<MariloFormItemRenderer>` inside the nested component. Consume the new `IFormItem` parameter in the `Item` parameter of the Renderer.
...
````RAZOR.skip-repl
<MariloFormItemRenderer Item="@NestedFormItem" />
...
* [Form Templates online demo](https://demos.marilo.com/blazor-ui/form/templates).
    ...
    <FormItemsTemplate Context="formContext">
        ...
        @foreach (IFormItemBase item in formContext.Items)
        {
            if (item is IFormGroup) // only if using FormGroups
            {
                ...
                <MariloFormGroupRenderer Group="@groupItem">
                    <Template Context="groupContext">
                        <div class="form-group-wrapper">
                            ...
                            @foreach (IFormItem singleItem in groupContext.Items)
                            {
                                <div class="form-item-wrapper">
                                    <MariloFormItemRenderer Item="@singleItem" />
                                </div>
                            }
                        </div>
                    </Template>
            ...
            {
                <div class="form-item-wrapper">
                    <MariloFormItemRenderer Item="@(item as IFormItem)" />
                </div>
            }
        }
...
<MariloForm Model="@Employee"
        ...
        <FormItem Field="@nameof(Person.BirthDate)">
            <Template>
                ...
                <div class="k-form-field-wrap">
                    <MariloDatePicker @bind-Value="@Employee.BirthDate"
                                       Format="d"
                                       Id="birth-date" />
                </div>
            </Template>
    ...
    <FormItemsTemplate Context="formContext">
        ...
        <div class="form-item-wrapper">
            ...
            <MariloFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Person.Id)) )" />
        </div>
        <div class="form-item-wrapper">
            @* Get Form item by index *@
            <MariloFormItemRenderer Item="@( formItems[1] )" />
        </div>
        <div class="form-item-wrapper">
            @* Get Form item by Id *@
            <MariloFormItemRenderer Item="@( formItems.First(x => x.Id == "last-name-item") )" />
        </div>
        <div class="form-item-wrapper">
            <MariloFormItemRenderer Item="@( formItems.Skip(3).First() )" />
        </div>
        ...
        {
            <div class="form-item-wrapper">
                <MariloFormItemRenderer Item="@( formItems.First(x => x.Field == nameof(Person.DriversLicense)) )" />
            </div>

            <div class="form-item-wrapper">
                <MariloFormGroupRenderer Group="@( formGroups.First(x => x.Id == "job-info-group") )">
                    <Template Context="groupContext">
                        @{
                            var groupItems = groupContext.Items.OfType<IFormItem>().ToList();
                        }
                        <MariloFormItemRenderer Item="@( groupItems.First(x => x.Field == nameof(Person.Team)) )" />
                        <MariloFormItemRenderer Item="@( groupItems.First(x => x.Field == nameof(Person.Salary)) )" />
                    </Template>
                </MariloFormGroupRenderer>
            </div>
        }
...
<MariloForm Model="@Employee"
    ...
    <FormItemsTemplate Context="formContext">
        @{
            ...
            {
                <div class="form-item-wrapper">
                    <MariloFormItemRenderer Item="@formItem" />
                </div>
            }
        }
    </FormItemsTemplate>
...
* [Online Demo: Form Templates](https://demos.marilo.com/blazor-ui/form/templates)
