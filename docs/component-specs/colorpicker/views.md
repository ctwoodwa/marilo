...
tags: marilo,blazor,colorpicker,views,overview
published: True
position: 0
...
The GradientView uses a [`MariloColorGradient` component](slug:colorgradient-overview). The following [ColorGradient attributes](slug:colorgradient-overview#colorgradient-parameters) are exposed in the `ColorPickerGradientView` tag:
...
The PaletteView uses a [`MariloColorPalette` component](slug:colorpalette-overview). The following [ColorPalette attributes](slug:colorpalette-overview#colorpalette-parameters) are exposed in the `ColorPickerPaletteView` tag:
...
* [ColorPicker Views Demo](https://demos.marilo.com/blazor-ui/colorpicker/views)
