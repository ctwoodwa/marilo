...
This article explains the events available in the Marilo FileManager for Blazor. They are grouped logically.
...
>tip You can also vote for the [FileManager feature request to expose a Proxy Url](https://feedback.marilo.com/blazor/1633629) for serving files from the server to the browser.
...
@code {
    ...
    private async Task OnDownloadHandler(FileManagerDownloadEventArgs args)
    {
        ...
        await Task.Delay(1); // simulate async operation
        string dummyFileContent = $"This file is a dummy version of {downloadedFile.Name}. It was downloaded with the Marilo Blazor FileManager.";
