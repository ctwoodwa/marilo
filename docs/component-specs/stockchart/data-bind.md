...
tags: marilo,blazor,stock,chart,databind,data,bind
published: True
position: 1
...
* `OHLC` and `Candlestick` - `OpenField`, `ClosedField`, `HighField` and `LowField` properties to the corresponding fields in the model that carry the values. Set the `DateField` parameter of the `<MariloStockChart>` to the `DateTime` property from the model that will be used for the x-axis.
