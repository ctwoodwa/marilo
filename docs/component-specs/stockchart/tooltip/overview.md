...
tags: marilo,blazor,stock,chart,tooltip,tooltips
published: True
position: 1
components: ["stockchart"]
---
# Tooltip for Marilo Blazor Stock Chart

The Marilo Stock Chart provides a tooltip for its data points. You can have settings specific to each `<StockChartSeries>`, common tooltip settings for all series, or a [shared](slug:stockchart-tooltip-shared) tooltip for all categories.
...
1. Inside the `<MariloStockChart>`, add the `<StockChartTooltip>`.
