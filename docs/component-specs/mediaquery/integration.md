...
tags: marilo,blazor,mediaquery,integration,chart,grid,form
published: True
position: 1
...
You can integrate the MariloMediaQuery component with our existing components. This article provides examples on the most common scenarios:
...
You can hide or more columns in the Grid based on the dimensions of the browser window by using the MariloMediaQuery component and the [Visible parameter](slug:components/grid/columns/bound#grid-bound-column-parameters) of the Grid column.

>tip You can use similar approach for the Marilo TreeList in order to hide some of the component columns on small devices. You can even replace the entire components with other components that have a simpler layout and limited functionality, such as a ListView, for small devices.
...
<MariloMediaQuery Media="(max-width: 1023px)" OnChange="@( (doesMatch) => IsMediumDown = doesMatch )"></MariloMediaQuery>
...
>note You can also see the <a href="https://github.com/marilo/blazor-ui/tree/master/chart/responsive-chart" target="_blank">Responsive Chart demo application</a> for additional examples.

````RAZOR
@* Resize the chart based on the browser size so it matches the corresponding responsive layout *@

<MariloMediaQuery Media="(max-width: 767px)" OnChange="@OnChangeHandler"></MariloMediaQuery>

<div style="width: @Width; height: @Height">

    <MariloChart Width="100%" Height="100%" @ref="@ChartRef">
...
@code {
    ...
    public MariloChart ChartRef { get; set; }
...
<MariloMediaQuery Media="@SmallScreenMediaQuery" OnChange="@( (bool matches) => { IsSmallScreen = matches; ConfigureForm(); } )" />
<MariloMediaQuery Media="@MediumScreenMediaQuery" OnChange="@( (bool matches) => { IsMediumScreen = matches; ConfigureForm(); } )" />
<MariloMediaQuery Media="@LargeScreenMediaQuery" OnChange="@( (bool matches) => { IsLargeScreen = matches; ConfigureForm(); } )" />
<MariloMediaQuery Media="@ExtraLargeScreenMediaQuery" OnChange="@( (bool matches) => { IsExtraLargeScreen = matches; ConfigureForm(); } )" />
...
* [Live Demo: MediaQuery and Grid Integration](https://demos.marilo.com/blazor-ui/mediaquery/grid-integration)
