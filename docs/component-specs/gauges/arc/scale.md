...
tags: marilo,blazor,arc,gauge,scale
published: True
position: 5
components: ["arcgauge"]
---
...
The scale of the arc gauge renders the values, pointers and labels. You can customize it by adding an instance of the `<ArcGaugeScale>` to the `<ArcGaugeScales>` collection, child tag of the `<MariloArcGauge>`. The `<ArcGaugeScale>` exposes the following parameters:
...
* [Live Demo: Arc Gauge - Scale Options](https://demos.marilo.com/blazor-ui/arcgauge/scale-options)
