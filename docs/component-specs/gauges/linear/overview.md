...
The <a href = "https://www.marilo.com/blazor-ui/linear-gauge" target="_blank">Marilo Linear Gauge for Blazor</a> represents [numerical values](slug:linear-gauge-pointers) on a [scale](slug:linear-gauge-scale) of ranges in a linear format.

## Creating Blazor Linear Gauge

1. Add the `<MariloLinearGauge>` tag.
...
<MariloLinearGauge @ref="@LinearGaugeRef" Height="@Height">
    <LinearGaugePointers>
...
@code {
    Marilo.Blazor.Components.MariloLinearGauge LinearGaugeRef { get; set; }
