...
tags: marilo,blazor,range,slider,overview
published: True
position: 0
...
The <a href="https://www.marilo.com/blazor-ui/rangeslider" target="_blank">Blazor Range Slider component</a> allows the user to select a value range by dragging its handles along the track. It provides templates, various configuration options, validation and keyboard navigation.

## Creating RangeSlider

1. Use the `MariloRangeSlider` tag to add the component to your razor page.
...
>caption Basic setup of the Marilo RangeSlider using two-way data binding
...
<MariloRangeSlider @bind-StartValue="@MinPrice"
...
````RAZOR
@code {
    MariloRangeSlider<decimal> TheRangeSlider { get; set; }
...
The RangeSlider lets you choose where its ticks will render. You can control that through the `TickPosition` parameter. It takes a member of the `Marilo.Blazor.SliderTickPosition` enum. Can be `Before`, `After`, `Both`(the default), `None`. For example, with the default horizontal slider, these values will render ticks above, below, both above and below, and no ticks.

## Orientation

You can customize the default horizontal orientation of the RangeSlider through its `Orientation` parameter. Takes a member of the `Marilo.Blazor.SliderOrientation` enum which contains `Horizontal`(the default) and `Vertical` options.
...
* [Live Demo: RangeSlider](https://demos.marilo.com/blazor-ui/rangeslider/overview)
* [Live Demo: RangeSlider Settings](https://demos.marilo.com/blazor-ui/rangeslider/customization)
