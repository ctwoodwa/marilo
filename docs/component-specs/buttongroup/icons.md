...
tags: marilo,blazor,Toggle,button,group,icon,sprite,image
published: True
position: 15
...
You can add a [Marilo Font or SVG icon](slug:common-features-icons) to the ButtonGroup items to illustrate its purpose by using the `Icon` parameter.
...
````RAZOR
<MariloButtonGroup SelectionMode="@ButtonGroupSelectionMode.Multiple">
