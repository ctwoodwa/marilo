...
tags: marilo, blazor, tilelayout, events
published: True
position: 30
...
This article explains the available events in the Marilo TileLayout for Blazor:
...
The TileLayout `OnResize` event fires when the user changes the dimensions of a tile. You can use the event to update the saved [TileLayout state](slug:tilelayout-state), or repaint a child component that needs it, such as the Marilo Chart.
...
@code {
    private MariloChart? ChartRef { get; set; }
