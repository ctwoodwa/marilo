...
tags: marilo,blazor,TreeList,filtering,filter,row
published: True
position: 5
components: ["treelist"]
---
...
To enable the filter row set the `FilterMode` property of the treelist to `Marilo.Blazor.TreeListFilterMode.FilterRow`.
...
>caption Filter Row in Marilo TreeList

````RAZOR
@* Filter row mode *@

<MariloTreeList Data="@Data" FilterMode="@TreeListFilterMode.FilterRow"
