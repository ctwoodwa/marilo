...
tags: marilo,blazor,treelist,drap,drop,dragdrop,row,rows,overview
published: True
position: 40
components: ["treelist"]
---
...
The Drag and Drop functionality for the TreeList rows allows you to move a row or a multitude of rows between different parents in the same TreeList or between different Marilo TreeList instances.
...
The `TreeListRowDraggableSettings` is a child tag under the `<TreeListSettings>`, which is a child tag of the `<MariloTreeList>`. It exposes the following parameters:
...
namespace MariloBlazorAppServer.Shared
{
...
<MariloTreeList Data="@SecondData"
                 ItemsField="@(nameof(Employee.DirectReports))"
                 Pageable="true" Width="850px"
                 RowDraggable="true"
...
@code {
    private MariloTreeList<Employee> FirstTreeList { get; set; }
...
You can drag and drop multiple rows in one or between multiple instances of the TreeList. To enable it, you should set the [`SelectionMode`](slug:treelist-selection-overview) parameter of the MariloTreeList to `TreeListSelectionMode.Multiple`. Then, if you drag a selected row, you will effectively drag all the selected rows.
...
* [Live Demo: TreeList Drag and Drop](https://demos.marilo.com/blazor-ui/treelist/drag-drop)
