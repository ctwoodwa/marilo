...
tags: marilo, blazor, treelist, editing
published: True
...
The Marilo TreeList for Blazor supports create, update, and delete operations (CRUD) with different modes and user experience. The component also supports built-in `DataAnnotations` or custom validation. This page describes:
...
1. Set the `EditMode` parameter to a [member of the `TreeListEditMode` enum](slug:marilo.blazor.treelisteditmode). The default `EditMode` parameter value is `None` and the built-in [`Add` and `Edit` commands](slug://treelist-columns-command#built-in-commands) don't work.
...
````RAZOR.skip-repl
<MariloTreeList EditMode="@TreeListEditMode.Popup"
...
* By using the [TreeList keyboard navigation](https://demos.marilo.com/blazor-ui/treelist/keyboard-navigation).
...
All events in the [Events table](#events), except `OnModelInit`, provide a [`TreeListCommandEventArgs`](slug:marilo.blazor.components.treelistcommandeventargs) event argument. It exposes the following properties:
...
You can control whether a newly added item appears at the top or bottom of the TreeList. Use the [`NewRowPosition`](https://www.marilo.com/blazor-ui/documentation/api/marilo.blazor.treelistnewrowposition) parameter to specify the position. This parameter does not affect Popup edit mode, which always displays a dialog for new items.
...
* [Online TreeList Demos](https://demos.marilo.com/blazor-ui/treelist/editing-inline)
