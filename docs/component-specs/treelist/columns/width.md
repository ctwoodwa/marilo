...
tags: marilo,blazor,treelist,column,width
published: True
position: 4
components: ["treelist"]
---
...
You can set the TreeList column `Width` parameter in any CSS unit, such as `px`, `%`, `vw`, `em`, `rem`. Unit-less `Width` values are not supported. You can read more on how to set sizes in Marilo components in the [Dimensions](slug:common-features/dimensions) article.
