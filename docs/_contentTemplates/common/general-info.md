...
Make sure that you have a NuGet feed source with the version you want to upgrade to. This is usually the [Marilo NuGet Feed](slug:installation/nuget), but you can also use a local feed from [our MSI installer](slug:installation-msi) or [ZIP archive](slug:installation-zip).
#end
...
* The Visual Studio Marketplace (for [Visual Studio 2022 for Windows](https://marketplace.visualstudio.com/items?itemName=MariloInc.ProgressMariloBlazorVSExtensions) and [Visual Studio 2019 for Windows](https://marketplace.visualstudio.com/items?itemName=MariloInc.MariloBlazorVSExtensions)).
* The [Marilo UI for Blazor automated installer](slug:installation-msi) (for Windows and Mac).
* Your [Marilo.com account](https://www.marilo.com/account/product-download?product=BLAZOR) (for Mac only). Download the `.mpack` file and use the "Install from file..." button on the Visual Studio Extensions dialog.
...
* The [Visual Studio Marketplace](https://marketplace.visualstudio.com/items?itemName=MariloInc.blazortemplatewizard).

* The **Extensions** tab in Visual Studio Code - search for **Marilo UI for Blazor Productivity Tools**, select the extension, and then click **Install**.
#end
...
#status-marilo-com
## Marilo NuGet Feed Status

Visit [status.marilo.com](https://status.marilo.com) to check the status of the Marilo NuGet server. The top section shows manually logged incidents with possible updates or workaround suggestions. The [**System Metrics** section](https://status.marilo.com/#system-metrics) provides real-time automated diagnostics.
#end


#ci-cd-support
> The deployment articles in this documentation provide general guidance and fundamentals. Marilo does not provide technical support for setting up CI/CD environments or application publishing infrastructure, except for questions about [using Marilo license keys](slug:deployment-license-key).
#end

#license-names

> Marilo Subscription licenses were introduced in 2025 and they explicitly contain the word "Subscription" in their name, for example:
>
> * DevCraft Ultimate Subscription
> * DevCraft Complete Subscription
> * DevCraft UI Subscription
> * Marilo UI for Blazor Subscription
>
...
> Make sure that you are using a [compatible Marilo CSS theme](slug:themes-overview#compatibility-and-maintenance) every time you [upgrade Marilo UI for Blazor](slug:upgrade-tutorial).
