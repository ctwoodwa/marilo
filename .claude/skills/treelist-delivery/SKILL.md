---
name: treelist-delivery
description: "ICM workspace: Coordinate MariloTreeList spec accuracy, Example UX completeness, and source+tests alignment. Stages: spec-review, example-ux, sync-check."
argument-hint: "[setup|status|spec|demo|sync|deliver]"
allowed-tools: Read Glob Grep
---

# TreeList Delivery — ICM Workspace Skill
...
1. Read `ICM/workspaces/treelist-delivery/CLAUDE.md` — this is the workspace routing layer
2. If `$ARGUMENTS` is provided, treat it as a **trigger keyword** and follow the Triggers table in CLAUDE.md
3. If no argument, read `ICM/workspaces/treelist-delivery/CONTEXT.md` for task routing
...
`ICM/workspaces/treelist-gap-analysis/` — source changes and test writing are delegated there.
