---
name: treeview-delivery
description: "ICM workspace: Coordinate MariloTreeView spec accuracy, Example UX completeness, and source+tests alignment. Stages: spec-review, example-ux, sync-check."
argument-hint: "[setup|status|spec|demo|sync|deliver]"
allowed-tools: Read Glob Grep
---

# TreeView Delivery — ICM Workspace Skill
...
1. Read `ICM/workspaces/treeview-delivery/CLAUDE.md` — this is the workspace routing layer
2. If `$ARGUMENTS` is provided, treat it as a **trigger keyword** and follow the Triggers table in CLAUDE.md
3. If no argument, read `ICM/workspaces/treeview-delivery/CONTEXT.md` for task routing
...
`ICM/workspaces/gap-analysis-resolution/` — TreeView graduated from the shared gap-analysis-resolution workspace.
